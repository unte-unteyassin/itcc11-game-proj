...
function startGameFromSplash() {
    ...
    if (sfxAkazaLanding) sfxAkazaLanding.volume = 0.9;
    if (sfxAkazaVoice) sfxAkazaVoice.volume = 0.9;
    if (sfxBreathing) sfxBreathing.volume = 0.55;
    if (sfxTotalConcentration) sfxTotalConcentration.volume = 0.8;
    // -------------------------

    if (BOSS_TEST_MODE) startAkazaIntro();
...
const sfxTotalConcentration = new Audio("audio/ability_call_5.mp3"); 

// --- AKAZA CUSTOM SKILL SOUNDS ---
...
// Cinematic Audio Tracks
const bgmTechniqueDeployment = new Audio("audio/technique_deployment.mp3");
bgmTechniqueDeployment.loop = false; bgmTechniqueDeployment.volume = 0.8;
const bgmBossTheme = new Audio("audio/boss_theme.mp3");
bgmBossTheme.loop = true; bgmBossTheme.volume = 0.6;

sfxAkazaLanding.volume = 0.9; sfxAkazaVoice.volume = 0.9;
const sfxBreathing = new Audio("audio/breathing.mp3");
sfxBreathing.loop = false; sfxBreathing.volume = 0.55;
...
let bossSmoke = []; // Dedicated smoke system for Akaza's Ultimate
...
const MAX_DEMONS = 11, MAX_PROJECTILES = 400, MAX_WATER_TRAILS = 70, MAX_FOAM = 140, MAX_JUMP_BURSTS = 10, MAX_SHOCKWAVES = 250, MAX_HEAL_TEXTS = 10, MAX_CLASH_SPARKS = 100, MAX_PARRY_SLASHES = 14;
const MAX_PROGRESS_SCORE = 65000;

const enemyMilestones = [
    { score: 15000, enemy: "SNIPER DEMON", hp: "1 HP", description: "A ranged demon that keeps distance and fires blood projectiles.", tip: "TIP: Dash in, use forms, or deflect its projectile back.", revealed: false },
    { score: 30000, enemy: "SWIFT DEMON", hp: "2 HP", description: "A fast demon that dashes through you and attacks quickly.", tip: "TIP: Guard, parry, then punish after its dash.", revealed: false },
    { score: 60000, enemy: "BRUTE DEMON", hp: "8 HP", description: "A heavy demon with armor, brutal swings, and shockwaves.", tip: "TIP: M1 is weak. Use Whirlpool or stronger forms.", revealed: false },
    { score: 65000, enemy: "???", hp: "???", description: "A terrifying presence approaches...", tip: "TIP: Something powerful waits beyond this score.", revealed: false }
];

let enemyRevealPopup = { active: false, timer: 0, maxTimer: 420, title: "", description: "", tip: "", hp: "", scoreNeeded: 0 };

function checkEnemyMilestones() {
    for (let i = 0; i < enemyMilestones.length; i++) {
        const m = enemyMilestones[i];
        if (!m.revealed && score >= m.score) { m.revealed = true; startEnemyRevealPopup(m); break; }
    }
}
function startEnemyRevealPopup(milestone) {
    enemyRevealPopup.active = true; enemyRevealPopup.timer = enemyRevealPopup.maxTimer; enemyRevealPopup.title = milestone.enemy; enemyRevealPopup.description = milestone.description; enemyRevealPopup.tip = milestone.tip; enemyRevealPopup.hp = milestone.hp; enemyRevealPopup.scoreNeeded = milestone.score;
    timeScale = 0.15; playSound(sfxReveal); startCameraShake(20, 5);
}
function updateEnemyRevealPopup() {
    if (!enemyRevealPopup.active) { timeScale = 1; return; }
    enemyRevealPopup.timer--; if (enemyRevealPopup.timer <= 0) { enemyRevealPopup.active = false; timeScale = 1; }
}
...
const player = {
    ...
    isGuarding: false, guardFrame: 0, perfectParryFrames: 30, guardCooldown: 0,
...
window.addEventListener("keydown", (e) => {
    ...
    if (e.code === "Space") { keys.Space = true; if (!player.isGuarding && player.guardCooldown <= 0 && !player.isAttacking && !player.isDashing) { player.isGuarding = true; player.guardFrame = 0; playSound(sfxGuardPull); } }
...
function saveHighScore(name, finalScore) { alert(`${name.toUpperCase()} SCORE: ${finalScore}`); location.reload(); }
...
function updateBossSmoke(dt) {
    for (let i = bossSmoke.length - 1; i >= 0; i--) {
        const s = bossSmoke[i];
        s.drift += 0.02 * dt;
        s.x += (s.vx + Math.sin(s.drift) * 1.5) * dt;
        s.y += s.vy * dt;
        s.vx *= Math.pow(s.friction, dt); 
        s.vy *= Math.pow(s.friction, dt);
        s.radius += s.grow * dt;
        s.alpha -= 0.0025 * dt; 
...
function drawBossSmoke() {
    ...
    bossSmoke.forEach((s) => {
        ...
        ctx.beginPath();
        ctx.arc(s.x - cameraX - s.radius * 0.2, s.y - s.radius * 0.2, s.radius * 0.5, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    });
}
...
function handlePlayerDamage(amount) {
    ...
    if (player.isGuarding) { finalDamage *= 0.35; player.breathing = Math.min(player.maxBreathing, player.breathing + 4); addFoamBurst(player.x + player.width / 2, player.y + player.height / 2, 6); }
    ...
    if (player.health <= 0) { player.health = 0; gameOver = true; setTimeout(() => { const tag = prompt(`SLAIN BY UPPER MOON 3! Final Score: ${score}\nEnter your Name Tag:`) || "SLAYER"; saveHighScore(tag, score); }, 50); }
}

function doPerfectParry(targetObject) {
    player.invincibleTimer = 35; player.guardCooldown = 0; combo += 6; comboTimer = 260; score += 1200;
    const healAmount = 8; player.health = Math.min(player.maxHealth, player.health + healAmount); player.breathing = Math.min(player.maxBreathing, player.breathing + 55); pushHealText(player.x + player.width / 2, player.y - 10, healAmount);
    if (targetObject) targetObject.stunnedTimer = 130;
    const px = player.x + player.width / 2; const py = player.y + player.height / 2;
    addFoamBurst(px, py, 28); pushClashSparks(px, py, 42, "#f97316"); pushClashSparks(px, py, 28, "#ffffff"); pushClashSparks(px, py, 18, "#38bdf8"); pushParrySlash(px, py); stunNearbyEnemies(px, py, 150); startCameraShake(14, 10); playSound(sfxParryClang); playSound(sfxPerfectParry);
    return true;
}

function stunNearbyEnemies(x, y, radius) {
    if (BOSS_TEST_MODE && akaza) { 
        // Only stun him if he IS NOT using Chaotic Afterglow
        if (akaza.currentAttack !== "chaotic_afterglow") {
            akaza.stunnedTimer = 40; 
...
function handleGuardOrParryAgainstPhysical(targetObject, damageAmount) {
    ...
    if (isPerfectParryWindow()) { doPerfectParry(targetObject); return; }
...
function startAkazaIntro() {
    demons.length = 0; projectiles.length = 0; shockwaves.length = 0; clashSparks.length = 0; parrySlashes.length = 0;
    score = 0; combo = 0; comboTimer = 0; spawnTimer = 0;
    player.x = 500; player.y = player.baseY; player.vy = 0; player.isGrounded = true; player.isAttacking = false; player.isGuarding = false; player.isDashing = false; player.isStunned = false; player.facing = "right";
    player.health = player.maxHealth;

    const spawnX = clamp(player.x + 620, 0, MAP_WIDTH - 70);
    akaza = {
...
if (akazaIntro.phase === 6) { 
    akazaIntro.cameraFocusX = player.x + player.width / 2; 
    akazaIntro.zoomTarget = 1.3; 
    akazaIntro.dialogue = "no"; 
    if (akazaIntro.timer >= 140) { akazaIntro.phase = 7; akazaIntro.timer = 0; playSound(bgmTechniqueDeployment);} 
    return; 
} 

    if (akazaIntro.phase === 7) { 
        akazaIntro.cameraFocusX = akaza.x + akaza.width / 2; 
        akazaIntro.zoomTarget = 1.4; 
        akazaIntro.dialogue = "i see"; 
        
        if (akazaIntro.timer >= 340) { 
             impactFrameTimer = 10; 
            startCameraShake(50, 45);
            akazaIntro.phase = 7.1; 
            akazaIntro.timer = 0; 
        } 
        return; 
    }
    ...
    if (akazaIntro.phase === 7.1) { 
        akazaIntro.compassRedAlpha = Math.min(0.85, akazaIntro.compassRedAlpha + 0.08 * realDt); 
        akazaIntro.compassRadius += (1100 - akazaIntro.compassRadius) * 0.08 * realDt;
        
        if (akazaIntro.timer >= 170) { 
            akazaIntro.phase = 7.2; 
            akazaIntro.timer = 0; 
        } 
        return; 
    }
    ...
    if (akazaIntro.phase === 7.2) { 
        akazaIntro.cameraFocusX = akaza.x + akaza.width / 2; 
        akazaIntro.zoomTarget = 1.4; 
        akazaIntro.dialogue = "Technique Deployment!"; 
        
        akazaIntro.compassRedAlpha = Math.min(0.85, akazaIntro.compassRedAlpha + 0.08 * realDt); 
        akazaIntro.compassRadius += (1100 - akazaIntro.compassRadius) * 0.08 * realDt;
        
        if (akazaIntro.timer >= 350) { 
            akazaIntro.phase = 9; 
            akazaIntro.timer = 0; 
            impactFrameTimer = 8;
            startCameraShake(25, 12);
        } 
        return; 
    }
    if (akazaIntro.phase === 9) { 
        akazaIntro.compassRedAlpha = Math.max(0, akazaIntro.compassRedAlpha - 0.05 * realDt); 
        akazaIntro.compassBlueAlpha = Math.min(0.85, akazaIntro.compassBlueAlpha + 0.04 * realDt);
        
        if (akazaIntro.timer >= 180) { 
            akazaIntro.phase = 9.1; 
            akazaIntro.timer = 0; 
        } 
        return; 
    }
    ...
    if (akazaIntro.phase === 10) {
        ...
        if (akazaIntro.letterbox <= 0 && akazaIntro.currentZoom <= 1.02) {
            ...
            bgmTechniqueDeployment.pause(); bgmBossTheme.play().catch(()=>{}); 
        }
    }
}
...
function updateAkazaBoss(realDt) {
    ...
    // MILESTONE-BASED ENRAGE SCALING
    ...
    // bossDt is for movement, realDt is for durations/timers!
    ...
    if (akaza.state === "idle") {
        ...
        // Pass BOTH bossDt (for moving) and realDt (for time/durations)
...
function startAkazaAttack(attackName) {
    ...
    switch(attackName) {
        ...
        case "chaotic_afterglow": 
            ...
            akaza.attackDuration = 1410; // Exactly 12 seconds in real time! (12s * 60fps * ~1.96 globalDt)
            akaza.flashRed = 45;
...
// Now accepts realDt to ensure time is accurate regardless of enrage multipliers
function processAkazaAttack(dt, realDt, dist, dir) {
    if (akaza.attackWindup > 0) {
        akaza.attackWindup -= realDt; // Use real time for windup!
    ...
    akaza.attackDuration -= realDt; // Use real time for duration!
    ...
    switch (akaza.currentAttack) {
        ...
        case "chaotic_afterglow": 
            ...
            // EXTREME BARRAGE - Shoots multiple per tick
            if (Math.floor(akaza.attackDuration) % 5 <= 1) { 
                
                // Spawn 4 projectiles and shockwaves instantly every trigger
                for (let i = 0; i < 4; i++) {
                    ...
                    const angle = Math.random() * Math.PI * 2;
                    const vSpeed = Math.random() * 20 + 25; // Highly randomized speeds (25 to 45)
                    ...
                    pushProjectile({ 
                        ...
                        radius: Math.random() * 20 + 20, // Mixed sizes
                        coreRadius: 12, 
                        color: "#ffffff", 
...
// Player attack interaction vs Boss
function handlePlayerAttackAgainstBoss() {
    ...
    if (hitBoss) {
        ...
        if (akaza.health <= 0) {
            akaza.health = 0; gameWon = true; score += 50000; addFoamBurst(akaza.x + akaza.width / 2, akaza.y + akaza.height / 2, 100); startCameraShake(50, 25); playSound(sfxBruteSlam);
            bgmBossTheme.pause(); 
            setTimeout(() => { const tag = prompt(`UPPER MOON SLAIN! Final Score: ${score}\nEnter your Name Tag:`) || "HASHIRA"; saveHighScore(tag, score); }, 3000);
        }
    }
}
...
function drawBlockyAkazaFalling(x, y) { ctx.save(); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 18, y + 24, 28, 34); ctx.fillStyle = "#f8fafc"; ctx.fillRect(x + 12, y + 54, 42, 18); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 2, y + 30, 18, 12); ctx.fillRect(x + 42, y + 30, 18, 12); ctx.fillRect(x + 15, y + 70, 12, 25); ctx.fillRect(x + 36, y + 70, 12, 25); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 17, y + 4, 30, 24); ctx.fillStyle = "#f472b6"; ctx.fillRect(x + 14, y, 36, 10); ctx.fillRect(x + 18, y - 5, 28, 8); ctx.strokeStyle = "#2563eb"; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(x + 20, y + 30); ctx.lineTo(x + 44, y + 50); ctx.moveTo(x + 44, y + 30); ctx.lineTo(x + 20, y + 50); ctx.stroke(); ctx.restore(); }
function drawBlockyAkazaKneeling(x, y) { ctx.save(); ctx.fillStyle = "#f8fafc"; ctx.fillRect(x + 34, y + 72, 32, 14); ctx.fillStyle = "#f8fafc"; ctx.fillRect(x + 9, y + 65, 22, 26); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 20, y + 35, 30, 34); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 5, y + 62, 18, 12); ctx.fillRect(x + 45, y + 60, 18, 12); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 19, y + 14, 30, 24); ctx.fillStyle = "#f472b6"; ctx.fillRect(x + 16, y + 8, 36, 10); ctx.fillRect(x + 21, y + 3, 26, 8); ctx.strokeStyle = "#2563eb"; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(x + 23, y + 42); ctx.lineTo(x + 48, y + 62); ctx.moveTo(x + 48, y + 42); ctx.lineTo(x + 23, y + 62); ctx.stroke(); ctx.fillStyle = "#38bdf8"; ctx.fillRect(x + 36, y + 24, 8, 4); ctx.restore(); }
function drawBlockyAkazaStandingUp(x, y, progress) { const kneelOffset = (1 - progress) * 20; ctx.save(); ctx.translate(0, kneelOffset); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 15, y + 62, 12, 33); ctx.fillRect(x + 38, y + 62, 12, 33); ctx.fillStyle = "#f8fafc"; ctx.fillRect(x + 10, y + 54, 45, 23); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 17, y + 25, 32, 36); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 3, y + 34, 13, 35); ctx.fillRect(x + 50, y + 34, 13, 35); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 18, y + 4, 30, 24); ctx.fillStyle = "#f472b6"; ctx.fillRect(x + 15, y - 2, 36, 10); ctx.fillRect(x + 20, y - 7, 26, 8); ctx.strokeStyle = "#2563eb"; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(x + 20, y + 32); ctx.lineTo(x + 47, y + 52); ctx.moveTo(x + 47, y + 32); ctx.lineTo(x + 20, y + 52); ctx.stroke(); ctx.fillStyle = "#38bdf8"; ctx.fillRect(x + 25, y + 14, 7, 4); ctx.fillRect(x + 38, y + 14, 7, 4); ctx.restore(); }
function drawBlockyAkazaStanding(x, y) { ctx.save(); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 15, y + 62, 12, 33); ctx.fillRect(x + 38, y + 62, 12, 33); ctx.fillStyle = "#f8fafc"; ctx.fillRect(x + 10, y + 54, 45, 23); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 17, y + 25, 32, 36); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 3, y + 34, 13, 35); ctx.fillRect(x + 50, y + 34, 13, 35); ctx.fillStyle = "#f3c7a2"; ctx.fillRect(x + 18, y + 4, 30, 24); ctx.fillStyle = "#f472b6"; ctx.fillRect(x + 15, y - 2, 36, 10); ctx.fillRect(x + 20, y - 7, 26, 8); ctx.strokeStyle = "#2563eb"; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(x + 20, y + 32); ctx.lineTo(x + 47, y + 52); ctx.moveTo(x + 47, y + 32); ctx.lineTo(x + 20, y + 52); ctx.stroke(); ctx.fillStyle = "#38bdf8"; ctx.fillRect(x + 25, y + 14, 7, 4); ctx.fillRect(x + 38, y + 14, 7, 4); ctx.restore(); }
...
function update() {
    ...
    if (keys.G && player.isGrounded && !player.isAttacking && !player.isGuarding && !player.isStunned && !akazaIntro.active) { dt = 0.2 * timeScale * globalDt; }
    updateTimers(dt, realDt);

    if (!akazaIntro.active) { updatePlayerMovement(dt); updatePlayerAttack(dt); handlePlayerAttackAgainstBoss(); }
    updateParticlesAndTrails(dt); updateJumpBursts(dt); updateShockwaves(dt); updateHealTexts(dt); updateProjectiles(dt); updateDemons(dt); updateCamera(dt); updateBossSmoke(dt);
}
...
function updatePlayerMovement(dt) {
    ...
    if (player.totalConcentrationActive && !player.isAttacking && !player.isStunned) player.breathing = Math.min(player.maxBreathing, player.breathing + 0.75 * dt);
    if (keys.G && player.isGrounded && !player.isAttacking && !player.isGuarding && !player.isStunned) { player.breathing = Math.min(player.maxBreathing, player.breathing + 1.2 * dt); player.bodyRotation = Math.sin(Date.now() / 40) * 0.05; }
    if (!player.isStunned && !keys.G) {
        if (keys.W && player.isGrounded && !player.isAttacking && !player.isGuarding) { player.vy = -13.4; player.isGrounded = false; player.jumpSquash = 10; playSound(sfxJump); addFoamBurst(player.x + player.width / 2, player.y + player.height, 14); pushJumpBurst(player.x + player.width / 2, player.y + player.height, "jump"); }
        const moveSpeed = player.isGuarding ? player.speed * player.guardSpeedMultiplier : player.speed;
        if (keys.Left && !player.isAttacking) { player.x -= moveSpeed * dt; player.facing = "left"; player.bodyRotation = player.isGrounded ? -0.08 : -0.18; } else if (keys.Right && !player.isAttacking) { player.x += moveSpeed * dt; player.facing = "right"; player.bodyRotation = player.isGrounded ? 0.08 : 0.18; } else if (!player.isAttacking && !player.isGuarding) { player.bodyRotation *= 0.84; }
    }
...
function updateCamera(dt) {
    let targetCamX; if (BOSS_TEST_MODE && akazaIntro.active && akaza) targetCamX = akazaIntro.cameraFocusX - canvas.width / 2; else targetCamX = player.x - canvas.width / 2;
...
function updatePlayerAttack(dt) {
    ...
    // CONTINUOUS MULTI-HIT LOGIC FOR FORMS
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (pr.deflected) {
            ...
            if (hitPlayer || hitAttack) {
                if (player.isGuarding || hitAttack) { pr.vx = -pr.vx * 1.5; pr.vy = (Math.random() - 0.5) * 3; pr.color = "#38bdf8"; pr.deflected = true; addFoamBurst(pr.x, pr.y, 10); playSound(sfxParryClang); pushClashSparks(pr.x, pr.y, 20, "#fb923c"); if (isPerfectParryWindow()) doPerfectParry(null); else { combo++; comboTimer = 240; score += 180; player.breathing = Math.min(player.maxBreathing, player.breathing + 10); } } else { playSound(sfxProjectileHit); handlePlayerDamage(pr.damage); projectiles.splice(i, 1); }
            }
        }
    }
}
...
// =====================================================
// REGULAR DEMONS (Disabled in BOSS TEST MODE)
// =====================================================
function spawnDemonPool() {
    if (demons.length >= MAX_DEMONS) return;
    const seed = Math.random(); let type = "grunt"; let speed = 1.4 + Math.random() * 1.2; let hp = 1 + Math.floor(Math.random() * 3); let color = "#7f1d1d"; let width = 42; let height = 55;
    ...
    demons.push({ id: demonIdCounter++, x, y: GROUND_Y - height, width, height, type, speed, health: hp, maxHealth: hp, color, jumpTimer: 0, shootTimer: 0, stunnedTimer: 0, attackWindup: 0, attackCooldown: 75, attackFlash: 0, attackRange: type === "brute" ? 70 : type === "swift" ? 52 : 48, attackHasHit: false, dashTimer: 0, dashCooldown: 120 + Math.random() * 60, dashDir: 1, dashDamageBoxTimer: 0, slamWindup: 0, slamCooldown: 190, slamActive: false });
}

function updateDemons(dt) {
    if (BOSS_TEST_MODE) return;
    spawnTimer += dt; if (spawnTimer >= 90) { spawnTimer = 0; spawnDemonPool(); }
    for (let i = demons.length - 1; i >= 0; i--) { const d = demons[i]; const beforeCount = demons.length; handlePlayerAttackAgainstDemon(i, d); if (demons.length < beforeCount) continue; if (d.stunnedTimer > 0) { d.stunnedTimer -= dt; d.attackWindup = 0; d.attackFlash = 0; continue; } if (d.attackCooldown > 0) d.attackCooldown -= dt; if (d.attackFlash > 0) d.attackFlash -= dt; if (d.dashCooldown > 0) d.dashCooldown -= dt; if (d.slamCooldown > 0) d.slamCooldown -= dt; moveDemon(d, dt); updateDemonAttack(d, dt); }
}
...
function killDemon(index, d) { addFoamBurst(d.x + d.width / 2, d.y + d.height / 2, 14); demons.splice(index, 1); combo++; comboTimer = 240; if (d.type === "brute") score += 600; else if (d.type === "swift") score += 400; else if (d.type === "sniper") score += 350; else score += 200; if (combo % 10 === 0) { player.health = Math.min(player.maxHealth, player.health + 4); player.breathing = Math.min(player.maxBreathing, player.breathing + 18); } }
...
function draw() {
    ...
    if (BOSS_TEST_MODE && akazaIntro.active) { 
    ...
    drawBackground(); drawAkazaCompass(); drawParticles(); drawJumpBursts(); drawShockwaves(); drawAkazaShockRings(); drawWaterTrails(); drawPlayer(); drawProjectiles(); drawEnemies(); drawAkaza(); drawAkazaSmoke(); drawBossSmoke(); drawCombatSparks(); drawHealTexts();
    ctx.restore();
...
function drawForm1Trail(t) { ctx.shadowColor = "#38bdf8"; ctx.fillStyle = "#1e40af"; ctx.fillRect(t.x - cameraX, t.y - 4, t.w, t.h + 8); ctx.fillStyle = "#06b6d4"; ctx.fillRect(t.x - cameraX, t.y + 4, t.w * 0.9, t.h - 8); for (let i = 0; i < t.w; i += 25) { drawWaveFoam(t.x - cameraX + i, t.y + (i % 3 === 0 ? 0 : t.h), 7 + (i % 4), t.alpha); } }
...
function drawSword() {
    ...
    else if (player.isAttacking) { const progress = player.attackFrame / player.maxAttackFrames; if (player.currentForm === 0) { const p = progress * Math.PI; if (player.m1Step === 1) swordAngle = player.facing === "right" ? -Math.PI / 3 + p : Math.PI + Math.PI / 3 - p; else if (player.m1Step === 2) swordAngle = player.facing === "right" ? -Math.PI / 2 + p * 1.2 : Math.PI + Math.PI / 2 - p * 1.2; else if (player.m1Step === 3) swordAngle = player.facing === "right" ? -Math.PI / 1.5 + p : Math.PI + Math.PI / 1.5 - p; else if (player.m1Step === 4) swordAngle = player.facing === "right" ? Math.PI / 4 - p : Math.PI - Math.PI / 4 + p; } else if (player.currentForm === 1) { swordAngle = player.facing === "right" ? 0 : Math.PI; } else if (player.currentForm === 2) { swordAngle = player.facing === "right" ? progress * Math.PI * 2 : -progress * Math.PI * 2; } else if (player.currentForm === 3) { swordAngle = player.facing === "right" ? progress * Math.PI * 8 : -progress * Math.PI * 8; } else if (player.currentForm === 4) { swordAngle = (player.facing === "right" ? 0 : Math.PI) + Math.sin(player.attackFrame * 0.15) * 1.2; } } else if (keys.G) { swordAngle = -Math.PI / 2; }
...
function drawTopBars() {
    const barW = 520; const barH = 24; const x = canvas.width / 2 - barW / 2; const hpY = 54; const breathY = 90;
    ...
    // Updating bar to use player.health
...
function drawEnemyRevealPopup() {
    ...
    const remainingPct = enemyRevealPopup.timer / enemyRevealPopup.maxTimer; const overlayAlpha = 0.45 + Math.sin(Date.now() / 80) * 0.08;
    ctx.save(); ctx.globalAlpha = overlayAlpha; ctx.fillStyle = "#000000"; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.globalAlpha = 1;
    const boxW = Math.min(900, canvas.width - 80); const boxH = 380; const x = canvas.width / 2 - boxW / 2; const y = canvas.height / 2 - boxH / 2;
    ctx.fillStyle = "#020617"; ctx.fillRect(x, y, boxW, boxH); ctx.strokeStyle = "#ef4444"; ctx.lineWidth = 7; ctx.strokeRect(x, y, boxW, boxH); ctx.strokeStyle = "#fbbf24"; ctx.lineWidth = 3; ctx.strokeRect(x + 12, y + 12, boxW - 24, boxH - 24);
    ctx.fillStyle = "#ef4444"; ctx.font = `bold 30px ${PIXEL_FONT}`; ctx.fillText("NEW THREAT REVEALED", x + 42, y + 64); ctx.fillStyle = "#ffffff"; ctx.font = `bold 42px ${PIXEL_FONT}`; ctx.fillText(enemyRevealPopup.title, x + 42, y + 130); ctx.fillStyle = "#fbbf24"; ctx.font = `bold 28px ${PIXEL_FONT}`; ctx.fillText(`HP: ${enemyRevealPopup.hp}`, x + 42, y + 180); ctx.fillStyle = "#cbd5e1"; ctx.font = `bold 23px ${PIXEL_FONT}`; wrapPixelText(enemyRevealPopup.description, x + 42, y + 225, boxW - 84, 32); ctx.fillStyle = "#38bdf8"; ctx.font = `bold 23px ${PIXEL_FONT}`; wrapPixelText(enemyRevealPopup.tip, x + 42, y + 300, boxW - 84, 32);
    ctx.fillStyle = "#1e293b"; ctx.fillRect(x + 42, y + boxH - 38, boxW - 84, 18); ctx.fillStyle = "#ef4444"; ctx.fillRect(x + 42, y + boxH - 38, (boxW - 84) * remainingPct, 18); ctx.strokeStyle = "#94a3b8"; ctx.lineWidth = 2; ctx.strokeRect(x + 42, y + boxH - 38, boxW - 84, 18); ctx.restore();
}
...
function gameLoop(currentTime) {
    ...
    // Cap dt so if the user tabs out, the game doesn't explode when they tab back in
...
// Start the loop with a starting timestamp
